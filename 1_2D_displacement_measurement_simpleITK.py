...
def displacement_measurement_2D(moving_image, fixed_image, supp_image, savepath=False):      
    ...
    # parameterMap['ImagePyramidSchedule'] = ['128 64 32 16 8 8 4 4 2 2 2 1 1 1 1']
    ...
    moving_image_translated_avg_z = np.nanmean(moving_image_translated, axis=0)
    fixed_image_avg_z = np.nanmean(fixed_image, axis=0)
    ...
    parameterMap['MaximumNumberOfIterations'] = ['500']
    parameterMap['FinalGridSpacingInVoxels'] = ['8 8']
...
def displacement_measurement_2Dt_main(moving_stack, fixed_3Dimage, supp_stack, savepath, radius, lower_threshold, sigma_smooth):
    ...
    p2 = np.percentile(fixed_3Dimage[-1, :, :], lower_threshold)
    p98 = np.percentile(fixed_3Dimage[-1, :, :], 99.9)
    ...
    for frame in np.arange(moving_stack.shape[0]):
        print("Removing background, stretching contrast and correcting for translation, frame " + str(frame))
        for current_z in np.arange(moving_stack.shape[1]):            
        ...
        if frame == 0:  # save registration parameters to file if the first frame is being analyzed
            moving_3Dimage_translated, supp_3Dimage_translated, moving_image_morphed, deformation_field = \
                                                    displacement_measurement_2D(moving_3Dimage_processed, fixed_3Dimage_processed, supp_3Dimage_processed, savepath=savepath)
        else:
            moving_3Dimage_translated, supp_3Dimage_translated, moving_image_morphed, deformation_field = \
                                                    displacement_measurement_2D(moving_3Dimage_processed, fixed_3Dimage_processed, supp_3Dimage_processed)                                            
        ...
        deformation_field_x[frame, :, :] = deformation_field[:, :, 0]
        deformation_field_y[frame, :, :] = deformation_field[:, :, 1]        
        

    return moving_stack_processed, moving_stack_morphed_avg_z, fixed_3Dimage_processed, supp_stack_processed, deformation_field_x, deformation_field_y



def main(path_BK, path_AK, path_supp_stack, savepath, finterval, pixelsize, radius=20, lower_threshold=95, sigma_smooth=1):
    ...
    BK, BK_morphed_avg_z, AK, supp_stack, d_x, d_y = displacement_measurement_2Dt_main(BK, AK, supp_stack, savepath, radius, lower_threshold, sigma_smooth)
    ...
    tifffile.imwrite(savepath + "/BK_morphed_avg_z.tif", 
                     BK_morphed_avg_z,
                     imagej=True,
                     resolution=(1/pixelsize, 1/pixelsize),
                     metadata={
    ...
    tifffile.imwrite(savepath + "/GAP-GFP_processed.tif", 
                      supp_stack,
                      imagej=True,
                      resolution=(1/pixelsize, 1/pixelsize),
                      metadata={
    ...
    tifffile.imwrite(savepath + "/GAP-GFP_avg_z.tif", 
                     supp_stack_avg_z,
                     imagej=True,
                     resolution=(1/pixelsize, 1/pixelsize),
                     metadata={
...
if __name__ == "__main__":
    ...
    lower_threshold = 90        # lower threshold percentile for bead stack
    ...
    # for position in [1, 2, 3, 4]:
        ...
        savepath = path + str(position) + "/TFM_data1/"

        if not os.path.exists(savepath):
            os.mkdir(savepath)
        main(path_BK, path_AK, path_supp_stack, savepath, finterval, pixelsize, radius=radius, lower_threshold=lower_threshold, sigma_smooth=sigma_smooth)

...
def main(path_supp_stack, savepath, forcemap_pixelsize, dmax=2, pmax=2):
    print("Loading image stack")
    supp_stack = io.imread(path_supp_stack)[:, :, :]  # / (2 ** 16)
    # load data
    d_x = np.load(savepath + "d_x.npy")    
    d_y = np.load(savepath + "d_y.npy")
    ...
    for frame in np.arange(no_frames):
        ...
        plot_image_with_forces(2*img_as_float(supp_stack[frame, :, :]), t_x[frame, :, :], t_y[frame, :, :], pmax, savepath + "GAP-GFP_forces" + str(frame), forcemap_pixelsize, frame=frame)


    # make_movies_from_images(savepath, "displacement", no_frames)
    # make_movies_from_images(savepath, "traction", no_frames)
    make_movies_from_images(savepath, "GAP-GFP_forces", no_frames)
